...
def train_once(
  ...
  with profiler.profile('Initializing model'):
    ...
    if base_workload == 'wmt' and 'wmt_1' in optimization_levels:
      logging.info("Performing `torch.compile` for wmt's loss_fn.")
      workload.loss_fn = torch.compile(workload.loss_fn)
    elif base_workload == 'ogbg' and 'ogbg_1' in optimization_levels:
      logging.info("Performing `torch.compile` for ogbg's loss_fn.")
      workload.loss_fn = torch.compile(workload.loss_fn)
    else:
      logging.info("No loss_fn optimization is set for the current workload.")
  ...
  if log_dir is not None:
    ...
    logging.info(f'Preemption count: {preemption_count}')
  ...
  if FLAGS.torch_profiler:
    ...
    prof = profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=False, with_stack=True)
    prof.__enter__()
